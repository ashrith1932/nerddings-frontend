...
if (repairedHome !== source) { fs.writeFileSync(homeFile, repairedHome); console.log("Repaired legacy HomeView JSX before build."); }
...
if (repairedProject !== projectSource) { fs.writeFileSync(projectFile, repairedProject); console.log("Repaired ProjectDetailSurface loading JSX before build."); }
...
const fallbackOldActions = `<div className="home-actions"><button className={post.liked ? "active" : ""} onClick={() => void action("like")}><Heart size={16} fill={post.liked ? "currentColor" : "none"} /><span>{post.likes}</span></button><button onClick={select}><MessageCircle size={16} /><span>{post.comments}</span></button><button className={post.reposted ? "active" : ""} onClick={() => void action("repost")}><Activity size={16} fill={post.reposted ? "currentColor" : "none"} /><span>{post.reposts}</span></button><button className={post.saved ? "active" : ""} onClick={() => void action("save")}><Bookmark size={16} fill={post.saved ? "currentColor" : "none"} /></button><button onClick={select}><Quote size={16} /></button><button onClick={async () => { await navigator.clipboard?.writeText(\`${window.location.origin}/post/${post.id}\`); }}><Share2 size={16} /></button></div>`;
...
if (feed.includes(oldActions)) feed = feed.replace(oldActions, newActions);
else if (feed.includes(fallbackOldActions)) feed = feed.replace(fallbackOldActions, newActions);
else console.log("HomeFeedSurface action footer already changed or pattern not found.");
feed = feed.replace("<div className=\"home-active-stats\"><span>{current.likes} likes</span><span>{current.comments} comments</span><span>{current.reposts} nerddings</span></div>", "<div className=\"home-active-stats\"><span>{current.likes} likes</span><span>{current.comments} comments</span><span>{current.reposts} amplifies</span><span>Views {current.views ?? 0}</span></div>");
