...
function PostComposer({ onClose }: { onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    apiFetch<{ data: { projects: Array<{ slug: string; name: string }> } }>(
      `/social/users/${encodeURIComponent(viewer.username)}/profile-live`
    )
      .then((r) => setProjects(r.data.projects.map((p) => ({ slug: p.slug, name: p.name }))))
      .catch(() => setProjects([]));
  }, [viewer?.username]);
  ...
  return (
    <div
      className="nerdd-create-backdrop"
    ...
    >
      <section className="nerdd-create-modal">
        ...
        <div className="nerdd-create-author">
          <span>{viewer?.name?.slice(0, 2).toUpperCase() || "N"}</span>
          <div>
        ...
        <textarea
          ...
          className="nerdd-create-textarea"
        />

        <label className="nerdd-create-media">
          <ImageIcon size={15} /> {files.length ? `${files.length} media` : "Add media"}
          <input
            type="file"
            accept="image/*,video/*"
            multiple
        ...
        {projects.length > 0 && (
          <label className="nerdd-create-project">
            <Rocket size={15} /> Mention project
        ...
        <footer>
          ...
          <button
            ...
            className="nerdd-create-publish-button"
          >
            {busy ? (
              <>
                <Loader2 className="nerdd-spin" size={15} />
                Publishing…
              </>
            ) : (
              <>
...
export default function CreateSurface() {
  ...
  return createPortal(
    <>
      {open && (
        <div className="nerdd-create-menu">
          <header>
            <div>
          ...
          <button
            ...
            className="nerdd-create-menu-item"
          >
            <span>
              <ImageIcon size={17} />
            </span>
            <div>
              <strong>Post</strong>
              <small>Share an update, idea or piece of work</small>
            </div>
          ...
          <footer>
            Only two creation types: <b>Post</b> and <b>Project</b>.
          </footer>
        </div>
      )}
