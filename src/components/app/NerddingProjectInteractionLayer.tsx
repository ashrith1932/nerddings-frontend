...
export default function NerddingProjectInteractionLayer(){
  ...
  useEffect(()=>{
    ...
    return()=>{document.removeEventListener("click",click,true);window.removeEventListener("popstate",assign);window.removeEventListener("nerdding:feed-refresh",assign);window.clearTimeout(timer);media.removeEventListener?.("change",resize)};
  },[]);
  if(!slug)return null;
  return <div className="nerdd-project-interaction"><style>{`.nerdd-project-interaction{position:fixed;inset:0;z-index:3050;background:rgba(25,20,16,.42);display:flex;justify-content:flex-end}.nerdd-project-panel{width:min(760px,94vw);height:100%;overflow:auto;background:#fffdf9;border-left:1px solid #ddd6cc;box-shadow:-25px 0 80px rgba(0,0,0,.22);position:relative}.nerdd-project-close{position:fixed;right:calc(min(94vw,760px) + 18px);top:18px;width:38px;height:38px;border:1px solid #ddd6cc;border-radius:50%;background:#fff;display:grid;place-items:center;z-index:10}@media(max-width:760px){.nerdd-project-panel{width:100%;border:0}.nerdd-project-close{display:none}.nerdd-project-back{display:flex!important}}.nerdd-project-back{display:none;width:100%;position:sticky;top:0;z-index:10;align-items:center;gap:7px;border:0;border-bottom:1px solid #e8e1d9;background:#fffdf9;padding:15px 18px;font-weight:800}`}</style><section className="nerdd-project-panel">{mobile&&<button className="nerdd-project-back" onClick={()=>setSlug(null)}><ArrowLeft size={16}/> Back</button>}<button className="nerdd-project-close" onClick={()=>setSlug(null)}><X size={18}/></button><ProjectDetailSurface slug={slug}/></section></div>;
}
