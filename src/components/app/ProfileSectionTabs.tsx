...
function BuildCard({ build }: { build: Build }) {
  return <article className="profile-build-card" onClick={() => nav(`/post/${encodeURIComponent(build.id)}`)}>
    <header className="profile-build-head">
      <div className="profile-build-author"><Avatar person={{ name: "", username: "", avatarUrl: null }} size={40} /><div><strong>Build update</strong><small>{build.projectName ? `${build.projectName} · ` : ""}{formatDate(build.createdAt)}{formatTime(build.createdAt) ? ` · ${formatTime(build.createdAt)}` : ""}</small></div></div>
      <time dateTime={build.createdAt}>{formatDate(build.createdAt)}</time>
    </header>
    ...
    <footer className="profile-build-actions">
      <span><Heart size={15} /> {build.likes}</span><span><MessageCircle size={15} /> {build.comments}</span><span><Activity size={15} /> {build.reposts}</span><span className="profile-build-spacer" /><span><span className="profile-build-muted"><Bookmark size={15} /> {build.saves}</span></span>
    </footer>
  </article>;
}
...
export default function ProfileSectionTabs({ username }: { username: string }) {
  ...
  return <section className="profile-section-tabs-wrap">
    ...
    <div className="profile-section-content">
      {tab === "builds" && <div className="profile-builds-list">{builds.length ? builds.map(build => <BuildCard key={build.id} build={build} />) : <div className="profile-tab-empty"><Activity size={20} /><strong>No builds yet</strong><span>Build updates will appear here like a feed.</span></div>}</div>}
