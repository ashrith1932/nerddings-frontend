...
export function LiveNotificationsRoute() {
  ...
  useEffect(() => { void load(); }, []);
  const markAll = async () => { try { await apiFetch("/notifications/read-all", { method: "POST" }); setItems(prev => prev.map(n => ({ ...n, readAt: n.readAt ?? new Date().toISOString() }))); } catch {} };
  const markRead = async (id: string) => { try { await apiFetch(`/notifications/${id}/read`, { method: "POST" }); setItems(prev => prev.map(n => n.id === id ? { ...n, readAt: n.readAt ?? new Date().toISOString() } : n)); } catch {} };
  const deleteNotification = async (id: string) => {
    const skipConfirm = localStorage.getItem('nerdding.notif-delete-confirm') === 'skip';
    if (!skipConfirm) {
      const confirmed = window.confirm('Delete this notification? (Check "Don\'t ask again" to skip in future)');
      if (!confirmed) return;
      localStorage.setItem('nerdding.notif-delete-confirm', 'skip');
    }
    try {
  ...
  return <div className="live-data-route"><style>{panelCss}{notifCardCss}</style><LiveNavMeta />
    ...
    {loading ? <div className="live-data-empty"><Loader2 size={20} /></div> : selectedItem ? (
      <div className="notif-detail-view" style={{padding: 20, background: '#fffdf9', borderRadius: 14, border: '1px solid #ded7cf'}}>
        <button className="outline-button" style={{marginBottom: 15, padding: '6px 12px', fontSize: 10}} onClick={() => setSelectedItem(null)}>← Back</button>
        <div style={{display: 'flex', alignItems: 'center', gap: 12, marginBottom: 15}}>
          {selectedItem.actor?.avatarUrl ? <img src={selectedItem.actor.avatarUrl} style={{width:48,height:48,borderRadius:'50%',objectFit:'cover'}} alt="" /> : <div style={{width:48,height:48,borderRadius:'50%',background:'#ebe5dd',display:'grid',placeItems:'center',fontSize:14,fontWeight:800,color:'#333'}}>{(selectedItem.actor?.name ?? "N").slice(0, 2).toUpperCase()}</div>}
          <div>
            <strong style={{fontSize: 14, color: '#201c19', display:'flex', alignItems:'center', gap:4}}>{selectedItem.actor?.name ?? "Nerdding"} {selectedItem.actor?.accountType === 'agent' && <VerifiedMark />}</strong>
            <div style={{fontSize: 11, color: '#8d837a'}}>@{selectedItem.actor?.username}</div>
          </div>
        </div>
        <p style={{fontSize: 13, color: '#403a34', lineHeight: 1.5, marginBottom: 20}}>{selectedItem.text}</p>
        <div style={{display: 'flex', gap: 8}}>
          {selectedItem.kind === "follow" && (
            <button className="outline-button" style={{color: '#c0473a', borderColor: '#f0d9d4', background: '#fff5f2'}} onClick={async () => {
              try { await apiFetch(`/users/${selectedItem.actor?.username}/block`, { method: "POST" }); alert("User blocked."); setSelectedItem(null); } catch { alert("Action blocked."); setSelectedItem(null); }
            }}>Block User</button>
          )}
          <button className="primary-button" onClick={() => navigateNotification(selectedItem)}>Go to Details</button>
        </div>
      </div>
    ) : items.length ? <div className="notif-list">
      {items.map((item) => {
        ...
        return <div className={`notif-card ${isUnread ? "notif-card--unread" : ""}`} key={item.id}>
          <div className="notif-card-left" onClick={() => { if (!isAffiliationRequest) { void markRead(item.id); setSelectedItem(item); } }}>
            {/* Avatar */}
          ...
          <div className="notif-card-body" onClick={() => { if (!isAffiliationRequest) { void markRead(item.id); setSelectedItem(item); } }}>
            <div className="notif-card-actor" style={{display:'flex',alignItems:'center',gap:4}}>{item.actor?.name ?? "Nerdding"}{item.actor?.accountType === 'agent' && <VerifiedMark />}</div>
          ...
          <div className="notif-card-actions">
            ...
            <button title="Delete" style={{border:'1px solid #f0d9d4',background:'#fff5f2',color:'#c0473a',borderRadius:7,padding:'4px 8px',fontSize:9,cursor:'pointer',marginLeft:4}} onClick={e => { e.stopPropagation(); void deleteNotification(item.id); }}>✕</button>
          </div>
        </div>;
      })}
  ...
  {creating && isAgent && (
    <div style={{background:'#fffdf9',border:'1px solid #ded7cf',borderRadius:14,padding:18,marginBottom:18}}>
      ...
      <form ref={fundFormRef} onSubmit={e => { e.preventDefault(); void createFundraising(new FormData(e.currentTarget)); }} style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:9}}>
        ...
        <label style={{display:'grid',gap:4,fontSize:9,fontWeight:800,color:'#7e756d'}}>Target Amount *<input name="targetAmount" type="number" min="1" required style={{border:'1px solid #ddd6cc',borderRadius:9,padding:'8px 9px',fontSize:10,background:'#fff',outline:'none'}} /></label>
        <label style={{display:'grid',gap:4,fontSize:9,fontWeight:800,color:'#7e756d'}}>Raised Amount<input name="raisedAmount" type="number" min="0" defaultValue="0" style={{border:'1px solid #ddd6cc',borderRadius:9,padding:'8px 9px',fontSize:10,background:'#fff',outline:'none'}} /></label>
