...
/** Profile Build Notes mirror the main home post structure without changing profile APIs. */
function PostCard({ post, user, onOpen }: { post: ProfilePost; user: ProfileUser; onOpen: (id: string) => void }) {
  ...
  return <article className="home-post" data-post-id={post.id}
    onClick={(event) => { if ((event.target as HTMLElement).closest("button,a,input,.nerdd-quote,video")) return; onOpen(post.id); }}>
    ...
    {post.quotePost?.author ? <article className="nerdd-quote" data-quote-post-id={post.quotePost.id} role="button" tabIndex={0}
