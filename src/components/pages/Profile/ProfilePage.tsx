...
function PostCard({ post, user, onOpen }: { post: ProfilePost; user: ProfileUser; onOpen: (id: string) => void }) {
  ...
  return <article className="home-post profile-build-card profile-post-row" data-post-id={post.id} onClick={(event) => { if ((event.target as HTMLElement).closest("button,a,.nerdd-quote,video")) return; onOpen(post.id); }}>
    <div className="home-post-head">
      <button className="home-author" onClick={(event) => { event.stopPropagation(); navigate(`/profile/${encodeURIComponent(user.username)}`); }}><UserAvatar user={user} /><span><strong>{user.name} {true && <VerifiedMark />}</strong><small>@{user.username} · {ago(post.createdAt)}</small></span></button>
    ...
    {post.quotePost?.author ? <article className="nerdd-quote profile-quote-card" role="button" tabIndex={0} onClick={(event) => { event.stopPropagation(); onOpen(post.quotePost!.id); }} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); event.stopPropagation(); onOpen(post.quotePost!.id); } }}><div className="nerdd-quote-label">QUOTED POST</div><div className="nerdd-quote-head"><span className="nerdd-quote-avatar">{post.quotePost.author.avatarUrl ? <img src={post.quotePost.author.avatarUrl} alt="" /> : initials(post.quotePost.author.name)}</span><span><strong>{post.quotePost.author.name} {true && <VerifiedMark />}</strong><small>@{post.quotePost.author.username} · {ago(post.quotePost.createdAt)}</small></span></div><div className="nerdd-quote-text">{post.quotePost.text}</div></article> : null}
...
export default function ProfileStandaloneView({ username }: { username: string }) {
  ...
  const [tab, setTab] = useState<"posts" | "projects" | "followers" | "following" | "interests">("posts");
  const [interests, setInterests] = useState<any[]>([]);
  const [interestsLoading, setInterestsLoading] = useState(false);
  useEffect(() => {
    if (tab !== 'interests') return;
    setInterestsLoading(true);
    apiFetch<{ data: any[] }>(`/users/${encodeURIComponent(username)}/interests`)
  ...
  return <div className="profile-page">
    ...
    <div className="profile-header"><UserAvatar user={snapshot.user} size={88} /><div className="profile-head-copy"><div className="profile-title-row"><div><h1>{snapshot.user.name} {true ? <VerifiedMark /> : null}</h1><p>@{snapshot.user.username}{snapshot.user.location ? ` · ${snapshot.user.location}` : ""}</p></div><div className="profile-actions">{!own ? <button className={following ? "outline-button" : "primary-button"} onClick={() => void toggleProfileFollowing(snapshot.user.id).then(setFollowing)}>{following ? <Check size={14}/> : <Plus size={14}/>} {following ? "Following" : "Follow"}</button> : <button className="outline-button"><SettingsIcon size={14}/> Edit profile</button>}</div></div><p className="profile-bio">{snapshot.user.bio || "Building in public on Nerdding."}</p></div></div>
    <div className="profile-section-tabs"><button className={tab === "posts" ? "active" : ""} onClick={() => setTab("posts")}>Build Notes <b>{snapshot.stats.posts}</b></button><button className={tab === "projects" ? "active" : ""} onClick={() => setTab("projects")}>Projects <b>{snapshot.stats.projects}</b></button><button className={tab === "followers" ? "active" : ""} onClick={() => setTab("followers")}>Followers <b>{snapshot.stats.followers}</b></button><button className={tab === "following" ? "active" : ""} onClick={() => setTab("following")}>Following <b>{snapshot.stats.following}</b></button><button className={tab === "interests" ? "active" : ""} onClick={() => setTab("interests")}>Interests</button></div>
    <div className="profile-content-grid"><div className="profile-section-content"><div className="profile-tab-search"><Search size={15}/><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={tab === "posts" ? "Search build notes" : `Search ${tab}`} /></div>{tab === "posts" ? <div className="profile-posts-scroll">{(visible as ProfilePost[]).map((post) => <PostCard key={post.id} post={post} user={snapshot.user} onOpen={openPost}/>)}{visible.length === 0 ? <div className="profile-tab-empty"><strong>No posts found</strong></div> : null}</div> : tab === "projects" ? <div className="profile-tab-grid">{(visible as ProfileProject[]).map((item) => <ProjectCard key={item.id} project={item}/>)}{visible.length === 0 ? <div className="profile-tab-empty"><strong>No projects found</strong></div> : null}</div> : tab === "interests" ? (<div className="profile-tab-grid">{interestsLoading ? (<div style={{padding:'20px',color:'var(--muted)',fontSize:11}}>Loading...</div>) : interests.length ? interests.map((item: any) => (<button key={item.id} className="profile-project-route" onClick={() => window.dispatchEvent(new CustomEvent('nerdding:open-project-inline', { detail: { slug: item.slug, id: item.id } }))}><div className="profile-project-art"><span style={{fontSize:22}}>⭐</span><span className="stage-pill">{item.stage}</span></div><div className="profile-project-copy"><h3>{item.name}</h3><p>{item.description || 'No description.'}</p><div><span>{item.interestCount} interested</span></div></div></button>)) : (<div className="profile-tab-empty"><strong>No interests yet</strong></div>)}</div>) : <PeopleList people={visible as ProfileSnapshot["followers"]}/>}</div><aside className="profile-right-rail">{activeProjectSlug ? <ProjectPreviewInline slug={activeProjectSlug} onClose={() => setActiveProjectSlug(null)} onOpenFull={() => { setActiveProjectSlug(null); navigate(`/project/${encodeURIComponent(activeProjectSlug)}`); }} /> : activePostId ? <ActivePost key={activePostId} id={activePostId} onClose={() => setActivePostId(null)} isPanel /> : <aside className="profile-affiliations-rail"><div className="profile-affiliations-rail-head"><div><div className="eyebrow">TRUSTED IDENTITY</div><h3>Affiliations</h3></div>{own && <button className="profile-tab-action" onClick={openAddAgent}><Plus size={14}/> Add Agent</button>}</div>{snapshot.affiliations.length ? (
            <div className="profile-affiliation-list">
              {snapshot.affiliations.map((a) => (
                <div className="profile-affiliation-modern" key={a.id}>
                  <strong style={{ display: "block", fontSize: "12px" }}>{a.name} {true && <VerifiedMark />}</strong>
