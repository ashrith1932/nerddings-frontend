...
function CommentNode({ node, depth, onReply }: { node: ThreadComment; depth: number; onReply: (parentId: string, body: string) => Promise<void> }) {
  const [replyOpen, setReplyOpen] = useState(false); const [collapsed, setCollapsed] = useState(false); const [text, setText] = useState(""); const [busy, setBusy] = useState(false);
